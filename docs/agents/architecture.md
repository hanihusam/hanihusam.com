...
| `render.server.ts`        | satori → resvg; parses its own params                             |
| `assets.server.ts`        | Cloudinary id → data URI, host-asserted                           |
...
- Rendered PNGs are cached in the SQLite cache (30 days) and served `immutable`.
  `X-Og-Cache: HIT|MISS` shows which path served a request.
